...
VULKAN_SDK_INSTALLER_URL = f'https://sdk.lunarg.com/sdk/download/{VULKAN_VERSION}/windows/vulkan_sdk.exe'
VULKAN_SDK_EXE_PATH = 'Vendors/VulkanSDK/VulkanSDK.exe'

def InstallVulkanSDK():
    print('Downloading {} to {}'.format(VULKAN_SDK_INSTALLER_URL, VULKAN_SDK_EXE_PATH))
    vulkanDir = os.path.dirname(VULKAN_SDK_EXE_PATH)
    if(not os.path.exists(vulkanDir)):
        os.makedirs(os.path.dirname(VULKAN_SDK_EXE_PATH))
...
def CheckVulkanSDK():
    if (VULKAN_SDK is None):
        ...
        print(f"You don't have the correct Vulkan SDK version! (Lucky requires {VULKAN_VERSION})")
        InstallVulkanPrompt()
        return False
...
VulkanSDKDebugLibsURL = f'https://files.lunarg.com/SDK-{VULKAN_VERSION}/VulkanSDK-{VULKAN_VERSION}-DebugLibs.zip'
OutputDirectory = "Vendors/VulkanSDK"
