...
class VulkanConfiguration:
    ...
    installVukanVersion = "1.3.268.0"
    vulkanDirectory = "Vendors/VulkanSDK"
    ...
    def __InstallVulkanSDK(cls):
        ...
        while not permissionGranted:
            reply = str(input("Would you like to install VulkanSDK {0:s}? [Y/N]: ".format(cls.installVukanVersion))).lower().strip()[:1]
            if reply == 'n':
                return
            permissionGranted = (reply == 'y')

        vulkanInstallURL = f"https://sdk.lunarg.com/sdk/download/{cls.installVukanVersion}/windows/VulkanSDK-{cls.installVukanVersion}-Installer.exe"
        vulkanPath = f"{cls.vulkanDirectory}/VulkanSDK-{cls.installVukanVersion}-Installer.exe"
